	...
	public class MappingProfile : Profile
	{
		public MappingProfile()
		{
			...
			CreateMap<Schedule, ScheduleDTO>()
				...
				.ForMember(dest => dest.StudentSchedules, opt => opt.MapFrom(src => src.StudentSchedules.Select(ss => new StudentSchedulesDTO { ScheduleId = ss.ScheduleId, StudentId = ss.StudentId, Student = new StudentDTO { Name = ss.Student.Name, Code = ss.Student.Code, Id = ss.Student.Id, Email = ss.Student.Email }, Status = ss.Status })));
			...
			CreateMap<StudentCourse, StudentCourseDTO>()
				.ForMember(dest => dest.Student, opt => opt.MapFrom(src => new StudentDTO
				{
					...
					StudentSchedules = src.Student.StudentSchedules.Select(ss => new StudentSchedulesDTO { ScheduleId = ss.ScheduleId, Status = ss.Status, Schedule = new ScheduleDTO { Date = ss.Schedule.Date,Slot = ss.Schedule.Slot, } }).ToList()
				}))
				.ForMember(dest => dest.Course, opt => opt.MapFrom(src => new CourseDTO
				{
					Code = src.Course.Code,
